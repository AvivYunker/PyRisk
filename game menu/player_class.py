...
class Player:
    def __init__(self, id_num, color, name, typed, cnt_init, trr_and_frc, allies):
        self.id_num = id_num # identification number
        self.color = color # color of player
        self.name = name # name of player (string)
        self.typed = typed # 0=COMP / 1=HUMAN
        self.cnt_init = 0 # the count of initial forces
        self.trr_and_frc = {} # territories and force (key=territory-id, value=number-of-forces)
        self.allies = [] # the allies of the player
    # get_color
    def assign_rgb (self):
        if (self.id_num == 0):
